...
> 
> empty_env_ds :: EnvDS
> empty_env_ds = EnvDS_Nil
>
> apply_env_ds :: EnvDS -> Var -> Value
> apply_env_ds (EnvDS_Nil) x          = error (x ++ " is not found.")
> apply_env_ds (EnvDS_Cons y v env) x
>   | x == y    = v
>   | otherwise = apply_env_ds env x
>
> extend_env_ds :: Var -> Value -> EnvDS -> EnvDS
> extend_env_ds x v f = EnvDS_Cons x v f
> 
...
> empty_env_fun =
>   EnvFun ( \x -> error (x ++ " is not found.") )
>
> apply_env_fun :: EnvFun -> Var -> Value
> apply_env_fun (EnvFun envfun) x = envfun x
> 
> extend_env_fun :: Var -> Value -> EnvFun -> EnvFun
> extend_env_fun x v (EnvFun envfun) =
>   EnvFun (\y -> if x==y then v else envfun x)
