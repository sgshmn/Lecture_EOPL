...
typechecker_tests =
  TestSuite
   [
     ...
     TYCK "pgm7b.inferred" (Just (TyFun TyInt TyInt)),

      -- circular type
      TYCK "circular_type.inferred" Nothing,
      TYCK "pgm11b_curried.inferred" (Just TyInt),
      TYCK "dont_infer_circular_type.inferred" Nothing,
      TYCK "polymorphic_type_1.inferred" (Just (TyFun (TyVar 1) (TyVar 2))),
      TYCK "polymorphic_type_2.inferred" (Just (TyFun TyInt (TyVar 1)))
   ]
